...
def clear_posts_text_to_parse(domain=''):  # clear text from words who can give as ValueError by parser
    posts = get_wall_posts(domain=domain)
    today = today_date()
    if 'rndm.club' in domain:
        for post in reversed(posts):
            post_text = post['text']
            if '|' in post_text.replace('год', '').replace('лет', ''):  # we split text to create list where [1] object will be date "day(1) mounth(january)" and here we miss posts without "|" char
        ...
        for post in reversed(posts):
            post_text = post['text']
            clear_concert_text = post_text.replace('лет', '').replace('лет', '')  # here we reaplace word 'years' becouse parser get mistake
